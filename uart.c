...
void uart_init(void) {
    ...
    // The debug UART's reference clock is fixed by firmware (commonly
    // 48MHz on Pi 5's dedicated debug UART). For 115200 baud:
    //   divisor = clock / (16 * baud) = 48000000 / (16*115200) = 26.041666...
    //   integer part = 26, fractional part = round(0.041666 * 64) = 3
    UART_IBRD = 26;
    UART_FBRD = 3;
